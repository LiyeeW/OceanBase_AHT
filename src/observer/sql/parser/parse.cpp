...
void relation_attr_init(RelAttr *relation_attr, const char *relation_name, const char *attribute_name) {
  ...
  relation_attr->aggregate_type = NON_AGG;
}
...
//-1 error date ,0 note date , pos is date;
int check_date(const char *v){
  int year, month, day;
  if(3 != sscanf(v, "%d-%d-%d", &year, &month, &day)) return 0;
  static int DAY[13] = {0,31,29,31,30,31,30,31,31,30,31,30,31};
  if(year<1970 || year>2038 || month<1 || month>12 || day<1 || day>DAY[month]) return -1;
  if(month==2 && day==29 && year%4) return -1;
  return year*10000+month*100+day;
}

int value_init_string(Value *value, const char *v) {
  //check whether date
  printf("CHARS\n");
  int d = check_date(v);
  if(d == -1) return 0;
