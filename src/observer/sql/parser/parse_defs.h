...
typedef enum {
  NON_AGG, MIN_AGG, MAX_AGG, AVG_AGG, CNT_AGG
}AggregateType;
...
typedef struct {
  ...
  size_t    group_num; 
  char *    group_table[MAX_NUM];
  char *    group_attr[MAX_NUM];
  size_t    order_num; 
  char *    order_table[MAX_NUM];
  char *    order_attr[MAX_NUM];
  int       order_type[MAX_NUM];
} Selects;
