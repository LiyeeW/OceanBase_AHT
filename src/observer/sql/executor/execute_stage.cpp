...
RC ExecuteStage::do_select(const char *db, Query *sql, SessionEvent *session_event) {
  ...
  if(has_agg){
    ...
    for(int i=0; i<main_tupleset->size(); i++){
      ...
      if(group_pos != -1){
        const std::shared_ptr<TupleValue> group_value_main = main_tupleset->get(i).get_pointer(group_pos_main);
        for(int line=0; line<(result_tuple_set->size()); line++){
          const TupleValue &group_value_result = result_tuple_set->get(line).get(group_pos);
          if(0 == group_value_main->compare(group_value_result)){
            result_row = line;
            break;
          }
        }
        if(result_row == -1){
          Tuple tuple;
          tuple.set_size(selects.attr_num+has_avg);
          tuple.set(group_pos, group_value_main);
          result_tuple_set->add(std::move(tuple));
      ...
      for(const auto &field: result_tuple_set->get_schema().fields()){
        ...
        const std::shared_ptr<TupleValue> new_value = main_value->compute(field.agg_type(), old_value);
      ...
      for(int i=0; i<result_tuple_set->size(); i++){
        for(int col=0; col<result_tuple_set->get_schema().fields().size(); col++){
          ...
          if(field.agg_type() == AVG_AGG){
            ...
            const std::shared_ptr<TupleValue> avg_value = sum_value->compute(NON_AGG, cnt_value);
