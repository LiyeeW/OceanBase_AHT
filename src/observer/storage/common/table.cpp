...
RC Table::drop(const char *path, const char *name, const char *base_dir) {
  ...
  if(remove(path) != 0) {
    LOG_ERROR("Failed to drop .table file.");
    return RC::SCHEMA_TABLE_EXIST;
  }

  std::string data_file = std::string(base_dir) + "/" + name + TABLE_DATA_SUFFIX;
  if(remove(data_file.c_str()) != 0) {
    LOG_ERROR("Failed to drop .data file.");
    return RC::SCHEMA_TABLE_EXIST;
  }
...
public:
  ...
  RC init(const char *attribute_name,const Value *value){
    const FieldMeta *field_meta = table_.table_meta_.field(attribute_name);
  ...
  RC update_record(Record *record) {
    ...
    memcpy(field_data, data_, field_meta_->len());
    rc = table_.record_handler_->update_record(record);
    if (rc == RC::SUCCESS) {
      updated_count_++;
    }
    return rc;
  ...
  RecordFileHandler *get_handler(){
    return table_.record_handler_;
  }
...
RC Table::update_record(Trx *trx, const char *attribute_name, const Value *value, ConditionFilter *filter, int *updated_count) {
  ...
  RC rc = updater.init(attribute_name, value);
  if(rc != RC::SUCCESS){
    LOG_INFO("update's attribute and value not matched");
    return rc;
  }
