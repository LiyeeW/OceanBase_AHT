...
public:
  ...
  virtual bool filter(const Tuple &tup) const = 0;
};
...
public:
  ...
  RC init(const Condition &condition);


  virtual bool filter(const Record &rec) const;
  virtual bool filter() const;
  virtual bool filter(const Tuple &tup) const;
...
public:
  ...
  RC init(ConditionFilter **filters, int filter_num);
private:
